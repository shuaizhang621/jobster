...
suniversity = ?, smajor = ? where semail = ?;";
$update_personal_info = $conn->prepare($sql_update_personal_info);
$update_personal_info->bind_param('ssssssss',$skey, $sfirstname, $slastname, $sgpa, $sphone,
    $suniversity, $smajor, $semail);
if ($update_personal_info->execute()){
    $response = "Student user ".$semail.": Profile updated successfully!";
}
else{
